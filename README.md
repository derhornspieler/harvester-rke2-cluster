...
Provisions a production-ready RKE2 Kubernetes cluster on Harvester via the Rancher API, using **Terraform** (IaC, state-managed) with the `rancher2` provider. Airgap-first architecture with golden image deployment, Harbor proxy-cache registry, Cilium CNI, and autoscaling worker pools.

## Overview

This project orchestrates the creation of a fully managed RKE2 cluster across Harvester hypervisor resources via Terraform:
...
Each managed cluster has its own `<cluster>.tfvars` file (e.g. `rke2-test.tfvars`, `rke2-prod.tfvars`) and its own Terraform state, segregated by `secret_suffix` in the `terraform-state` namespace on Harvester's Kubernetes backend. Switch between clusters via `terraform init -reconfigure -backend-config="secret_suffix=<cluster>"`.
...
- `.env` — Environment file consumed by `nuke-cluster.sh`
- `deploy-terraform/terraform.tfvars` — Default tfvars file for Terraform

### Multi-cluster credential preparation

Run `prepare.sh --env` once per cluster to generate per-cluster credentials without overwriting:

```bash
./prepare.sh --env .env.rke2-test
./prepare.sh --env .env.rke2-prod
./prepare.sh --env .env.rke2-test -r   # refresh test only
```

Outputs (with `--env`):

- `harvester-cloud-provider-kubeconfig-<cluster>` — namespace-scoped Harvester SA token
- `kubeconfig-harvester-cloud-cred-<cluster>.yaml` — cluster-cred kubeconfig
- The target env file's pointer variables are updated in place (these env files exist only for `nuke-cluster.sh` to consume during emergency teardown — Terraform itself reads from `<cluster>.tfvars`)

The Harvester management kubeconfig (`kubeconfig-harvester.yaml`) is shared across RKE2 clusters because it points at the Harvester install itself.

Default mode (no `--env`) keeps the original single-cluster behavior with unsuffixed filenames.

### Step 2: Configure Your tfvars File

Copy the example and fill in cluster-specific values. Each cluster has its own tfvars file:

```bash
cp deploy-terraform/terraform.tfvars.example deploy-terraform/rke2-test.tfvars
cp deploy-terraform/terraform.tfvars.example deploy-terraform/rke2-prod.tfvars
# edit each with cluster-specific values
```

Edit `deploy-terraform/rke2-<cluster>.tfvars` with your configuration:
...
cilium_image_registry                      = "<your-harbor>/quay.io"
private_ca_pem                             = "-----BEGIN CERTIFICATE-----\n..."
deploy_operators                           = false
```

All tfvars files (including per-cluster variants) are gitignored — they contain secrets.
...
# One-time per workspace switch: bind backend to this cluster's state secret
...
Updates (Kubernetes version bumps, scale changes, registry list edits, etc.) are just another `terraform apply` against the same tfvars. Terraform is declarative — there is no separate "update" mode.

To force a full pool roll (e.g., to redeploy with a new golden image or pick up a refreshed cloud-init), bump `rotation_marker` in the tfvars file and re-apply. To rotate just the control plane, run `terraform taint` on the controlplane `rancher2_machine_config_v2` and apply.
...
```bash
cd deploy-terraform
terraform destroy -var-file=rke2-prod.tfvars
```

For stuck CAPI/Harvester resources after a failed destroy, fall back to `./nuke-cluster.sh --env .env.rke2-prod`.

Cluster provisioning typically takes 20–40 minutes. Monitor via Rancher UI or kubectl with the returned kubeconfig.

VM anti-affinity is enforced by default: control plane VMs use hard anti-affinity (each CP VM must land on a distinct Harvester host for etcd quorum safety), and worker pools use best-effort preferred spreading. See [VM Anti-Affinity / Host Spreading](./docs/operations.md#14-vm-anti-affinity--host-spreading) for the rebalance procedure for existing clusters.
...
Configuration is managed via per-cluster tfvars files in `deploy-terraform/<cluster>.tfvars` (see `deploy-terraform/terraform.tfvars.example` for template).
...
State is stored in a Kubernetes backend on the Harvester cluster (`terraform-state` namespace) — NOT on your local machine. Each cluster has its own state secret, segregated by `secret_suffix`. This allows state sharing across team members and CI/CD pipelines.

Switch active workspace via `terraform init -reconfigure -backend-config="secret_suffix=<cluster>"`.

## Day 2 Operations

### Scaling Workers

Edit `deploy-terraform/<cluster>.tfvars` and adjust `*_min_count` / `*_max_count`:
...
Edit `deploy-terraform/<cluster>.tfvars` and change `kubernetes_version`:
...
Cloud credentials stored in `kubeconfig-harvester-cloud-cred.yaml` and `harvester-cloud-provider-kubeconfig` are NOT deleted by `terraform destroy`, allowing you to recreate the cluster without re-running `prepare.sh`.

`terraform destroy` performs:

1. Deletes the RKE2 cluster from Rancher API
2. Waits for VMs to be deleted by Cluster API (asynchronous)
3. Removes managed Terraform resources (machine configs, EFI patches, operators, etc.)

For VMs / CAPI Machines that get stuck mid-teardown, fall back to `nuke-cluster.sh` (next section). It performs the deeper finalizer cleanup that Terraform doesn't handle.

**Note**: This removes all RKE2 VMs from Harvester but leaves the Harvester cluster and networks intact.

### Nuclear Cleanup

If `terraform destroy` hangs or leaves orphaned resources, use `nuke-cluster.sh`:
...
├── .env.<cluster>                     # Per-cluster env files for nuke-cluster.sh (gitignored)
│
├── deploy-terraform/                  # Terraform IaC deployment (single source of truth)
...
│   ├── migration_policy.tf            # Per-pool KubeVirt MigrationPolicy
│   ├── vm_eviction_strategy.tf        # Per-pool VM eviction strategy
│   ├── cloud_credential.tf            # Harvester cloud credential
│   ├── image.tf                       # Golden image data source
│   ├── efi.tf                         # EFI patches for initial bootstrap
│   ├── operators.tf                   # Operator deployment manifests
│   ├── rke2-test.tfvars               # rke2-test cluster config (gitignored)
│   ├── rke2-prod.tfvars               # rke2-prod cluster config (gitignored)
│   ├── terraform.tfvars.example       # Template for cluster tfvars
│   ├── .terraform/                    # Provider cache (gitignored)
│   └── .terraform.lock.hcl            # Terraform dependency lock file
│
├── operators/                         # Shared operators directory
│   ├── images/                        # OCI image tarballs (gitignored)
│   │   ├── node-labeler-v0.2.0-amd64.tar.gz
│   │   └── storage-autoscaler-v0.2.0-amd64.tar.gz
│   ├── templates/                     # Deployment YAML templates (.tftpl)
│   │   ├── cluster-autoscaler-deployment.yaml.tftpl
│   │   ├── node-labeler-deployment.yaml.tftpl
│   │   └── storage-autoscaler-deployment.yaml.tftpl
│   ├── manifests/                     # Kubernetes manifests
│   │   ├── cluster-autoscaler/        # Static RBAC, namespace, PDB
│   │   ├── node-labeler/              # Static RBAC, namespace
│   │   ├── storage-autoscaler/        # Static RBAC, namespace
...
│   └── push-images.sh                 # Pushes custom operator images to Harbor
│
...
### For Terraform Changes (deploy-terraform/)

1. Use `terraform fmt deploy-terraform/` to format all `.tf` files
2. Validate with `cd deploy-terraform && terraform validate`
3. Test against a non-production cluster first (`rke2-test.tfvars`) before touching prod
4. Keep variable descriptions clear and concise
5. Update `deploy-terraform/terraform.tfvars.example` when adding new variables
...
- Clear description of changes (which variables, which features)

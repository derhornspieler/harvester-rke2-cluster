...
graph LR
    ...
    subgraph BootstrapPhase["Cluster Startup"]
        BootstrapRegistry["Bootstrap Registry<br/>(e.g., <your-harbor>)<br/>---<br/>Used only at cluster<br/>provisioning time"]
    end

    subgraph HarborPhase["After Cluster Ready"]
        Harbor["Harbor Proxy-Cache<br/>(<your-harbor>)<br/>---<br/>Full Harbor deployment<br/>with upstream mirrors"]
    end
...
2. **Bootstrap Phase (Cluster Startup)**
   - Registry endpoint: `var.bootstrap_registry` (e.g., `<your-harbor>`)
...
3. **Harbor Proxy-Cache Phase (After Cluster Ready)**
   ...
   - Same mirror rewrites but now endpoint is `var.harbor_fqdn` (e.g., `<your-harbor>`)
...
- **Registry**: Pushed to Harbor via `crane` CLI: `<your-harbor>/library/node-labeler:v0.2.0`
...
- **Transparent Proxying**: No code changes needed; containerd automatically routes `ghcr.io/*` → `<your-harbor>/ghcr/*`

...
1. **Registry FQDN misconfigured (bare IP instead of FQDN)**
   ...
   - Symptom: `curl https://10.0.0.100/v2/` returns 404, but `curl -H "Host: <your-harbor>" https://10.0.0.100/v2/` works
   - Solution: Use FQDN in `terraform.tfvars`: `bootstrap_registry = "<your-harbor>"`
   - Ensure `/etc/hosts` or DNS maps `<your-harbor>` → IP address
...
**Permanent fix (already applied 2026-04-26)**: chart_values override in `deploy-terraform/cluster.tf` `rke2-cilium` block pins all Cilium component images via the `cilium_image_registry`, `cilium_image_tag`, and `hubble_ui_image_tag` variables (set per-cluster in tfvars to your registry/proxy-cache):

```hcl
"rke2-cilium" = {
  ...
  image    = { repository = "${var.cilium_image_registry}/cilium/cilium",         tag = var.cilium_image_tag, useDigest = false }
  operator = { image = { repository = "${var.cilium_image_registry}/cilium/operator", tag = var.cilium_image_tag, useDigest = false } }
  hubble   = {
    relay = { image = { repository = "${var.cilium_image_registry}/cilium/hubble-relay", tag = var.cilium_image_tag, useDigest = false } }
    ui    = {
      backend  = { image = { repository = "${var.cilium_image_registry}/cilium/hubble-ui-backend", tag = var.hubble_ui_image_tag, useDigest = false } }
      frontend = { image = { repository = "${var.cilium_image_registry}/cilium/hubble-ui",         tag = var.hubble_ui_image_tag, useDigest = false } }
    }
  }
}
```

...
# COMMENTED OUT 2026-04-19: tied to the disabled vm-eviction-reconciler CronJob
# in vm_eviction_strategy.tf. Re-enable alongside the CronJob if the descheduler
# is reintroduced before Kyverno mutating-webhook enforcement lands.
# variable "manage_vm_eviction_reconciler" {
#   description = "Whether this workspace manages the Harvester-side vm-eviction-reconciler SA/Role/Binding/CronJob. Set true in exactly ONE workspace (typically rke2-test); other workspaces set this to false to avoid SA/CR collisions since it's a cluster-scoped shared resource."
#   type        = bool
#   default     = true
# }
...
variable "cluster_autoscaler_chart_repository" {
  description = "OCI repo path for cluster-autoscaler chart (e.g. oci://harbor.example.com/kubernetes.github.io/autoscaler). Must exist before first apply."
  type        = string
  default     = "oci://harbor.example.com/kubernetes.github.io/autoscaler"
}
...
variable "cluster_autoscaler_image_repository" {
  description = "Container image repo for cluster-autoscaler (Harbor proxy-cache of registry.k8s.io)."
  type        = string
  default     = "harbor.example.com/registry.k8s.io/autoscaling/cluster-autoscaler"
}
...
variable "storage_autoscaler_chart_repository" {
  description = "OCI repo path for storage-autoscaler chart (in-house; lives in Harbor /library project)."
  type        = string
  default     = "oci://harbor.example.com/library"
}
...
variable "storage_autoscaler_image_repository" {
  description = "Container image repo for storage-autoscaler (Harbor /library)."
  type        = string
  default     = "harbor.example.com/library/storage-autoscaler"
}
...
# -----------------------------------------------------------------------------
# VM eviction reconciler (Harvester-side CronJob)
# -----------------------------------------------------------------------------
# Set true in EXACTLY ONE workspace's tfvars (rke2-test). Other workspaces
# (rke2-prod) must keep this false to avoid duplicate ServiceAccount /
# ClusterRole resource conflicts on the shared Harvester underlay. The
# CronJob reconciles VM eviction strategy across BOTH downstream cluster
# namespaces (rke2-test, rke2-prod) regardless of which workspace owns it.
variable "manage_vm_eviction_reconciler" {
  description = "If true, this workspace owns the shared Harvester-side VM evictionStrategy CronJob. Set true for ONE workspace only."
  type        = bool
  default     = false
}

...
cmd_apply() {
  ...
  if [[ ! -d .terraform ]]; then
    needs_init=true
  elif [[ "${TFVARS_FILE}" -nt .terraform ]]; then
    log_info "${TFVARS_FILE} is newer than .terraform/ — forcing re-init..."
    needs_init=true
...
cmd_validate() {
  ...
  if [[ ! -d .terraform ]] || ! TF validate -no-color &>/dev/null; then
    tf_init_backend
  fi
  ...
  [[ -n "$vm_namespace" ]] || die "vm_namespace not set in ${TFVARS_FILE}"

  log_info "Checking golden image '${golden_image}' exists in namespace '${vm_namespace}'..."
  if $KUBECTL get virtualmachineimages.harvesterhci.io "${golden_image}" -n "${vm_namespace}" &>/dev/null; then
    ...
    die "Golden image '${golden_image}' not found in namespace '${vm_namespace}' — upload before apply"
  fi
}
...
post_destroy_cleanup() {
  ...
  # Stale cloud credentials matching the cluster name
  local stale_creds
  stale_creds=$(curl -sk -H "$auth_header" "${rancher_url}/v3/cloudCredentials" 2>/dev/null \
    | jq -r ".data[]? | select(.name | startswith(\"${cluster_name}\")) | .id" 2>/dev/null || true)
  if [[ -n "$stale_creds" ]]; then
    while IFS= read -r cred_id; do
      [[ -z "$cred_id" ]] && continue
      curl -sk -X DELETE -H "$auth_header" "${rancher_url}/v3/cloudCredentials/${cred_id}" >/dev/null 2>&1 || true
    done <<< "$stale_creds"
    log_ok "Cleaned stale cloud credentials"
  fi
}

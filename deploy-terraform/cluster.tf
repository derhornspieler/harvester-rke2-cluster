  ...
  rke_config {
    ...
    chart_values = yamlencode({
      ...
      "rke2-cilium" = {
        ...
        # Pin Cilium tag in variables.tf (cilium_image_tag, default v1.19.3) to
        # avoid the BPF lb slot-gap bug present in v1.19.1 (RKE2 v1.34.5+rke2r1
        # bundles 1.19.1). Upstream fix lives in v1.19.2+. Registry prefix
        # comes from var.cilium_image_registry — point at your Harbor proxy
        # in tfvars for airgapped sites; defaults to upstream quay.io.
        image = {
          repository = "${var.cilium_image_registry}/cilium/cilium"
          tag        = var.cilium_image_tag
          useDigest  = false
        }

        operator = {
          replicas = 1
          # REQUIRED with kubeProxyReplacement=true + k8sServiceHost=127.0.0.1.
          # The chart bundled with our RKE2 auto-defaults this to true for our
          # config, but other RKE2/chart variants (notably airgapped sites with
          # different bundled charts) may default to false → operator pod runs
          # in pod-network → cannot dial 127.0.0.1:6443 (Cilium socket-LB does
          # not rewrite loopback) → operator crashloops on
          # gatewayAPI.initGatewayAPIController context deadline (~30s) and
          # the cluster never finishes provisioning. Reproduced 2026-05-07
          # by forcing hostNetwork=false on rke2-test. Make it explicit so
          # the GitHub-mirror code is portable across RKE2 versions.
          hostNetwork = true
          image = {
            ...
            # set repository to ".../cilium/operator" and the chart appends
            # "-generic" automatically.
            repository = "${var.cilium_image_registry}/cilium/operator"
            tag        = var.cilium_image_tag
            useDigest  = false
          }
        }
        ...
        hubble = {
          ...
          relay = {
            enabled = true
            image = {
              repository = "${var.cilium_image_registry}/cilium/hubble-relay"
              tag        = var.cilium_image_tag
              useDigest  = false
            }
          }
          ui = {
            ...
            backend = {
              image = {
                repository = "${var.cilium_image_registry}/cilium/hubble-ui-backend"
                tag        = var.hubble_ui_image_tag
                useDigest  = false
              }
            }
            frontend = {
              image = {
                repository = "${var.cilium_image_registry}/cilium/hubble-ui"
                tag        = var.hubble_ui_image_tag
                useDigest  = false
              }
            }

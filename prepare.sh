...
Options:
  ...
  -r, --refresh-credentials Refresh only credentials and kubeconfigs without
                            touching other tfvars values. Requires an
                            existing tfvars file (falls back to full setup
                            if missing).
  --cluster <name>          Per-cluster mode (parallels terraform.sh). Reads
                            from deploy-terraform/<name>.tfvars and writes
                            credentials back to that file. Output kubeconfigs
                            are suffixed with <name>. Combinable with -r.
                            Example: --cluster rke2-prod -r
  --env FILE                Path to env file (default: .env). Legacy multi-
                            cluster mode. Mutually exclusive with --cluster.
...
# -----------------------------------------------------------------------------
# Read a simple key = "value" from a tfvars file.
# Same pattern as _get_tfvar_value in terraform.sh / nuke-cluster.sh.
#   read_tfvar <key>             — uses ${ACTIVE_TFVARS} (set by refresh_credentials)
#   read_tfvar <key> <file>      — explicit file path
# -----------------------------------------------------------------------------
read_tfvar() {
  local key="$1"
  local file="${2:-${ACTIVE_TFVARS:-${SCRIPT_DIR}/deploy-terraform/terraform.tfvars}}"
  awk -F'"' "/^${key}[[:space:]]/ {print \$2}" "${file}" 2>/dev/null || echo ""
}
...
refresh_credentials() {
  ...
  if [[ -n "${CLUSTER_FLAG:-}" ]]; then
    # --cluster mode: read from deploy-terraform/<cluster>.tfvars
    ACTIVE_TFVARS="${SCRIPT_DIR}/deploy-terraform/${CLUSTER_FLAG}.tfvars"
    [[ -f "${ACTIVE_TFVARS}" ]] || die "tfvars file not found: ${ACTIVE_TFVARS}"
    log_info "Reading existing settings from ${ACTIVE_TFVARS#${SCRIPT_DIR}/}..."
    RANCHER_URL=$(read_tfvar rancher_url)
    CLUSTER_NAME=$(read_tfvar cluster_name)
    VM_NAMESPACE=$(read_tfvar vm_namespace)
    # Per-cluster output kubeconfig paths (parallel to terraform.sh)
    CLOUD_PROVIDER_KUBECONFIG_OUT="${SCRIPT_DIR}/harvester-cloud-provider-kubeconfig-${CLUSTER_NAME}"
    CLOUD_CRED_KUBECONFIG_OUT="${SCRIPT_DIR}/kubeconfig-harvester-cloud-cred-${CLUSTER_NAME}.yaml"
    HARVESTER_KUBECONFIG_OUT="${SCRIPT_DIR}/kubeconfig-harvester.yaml"
    ENV_OUT=""
    ...
    # Backward-compat: read from .env or deploy-terraform/terraform.tfvars
    resolve_env_target
    if [[ -f "${SCRIPT_DIR}/.env" ]]; then
      ...
      ACTIVE_TFVARS="${SCRIPT_DIR}/deploy-terraform/terraform.tfvars"
      RANCHER_URL=$(read_tfvar rancher_url)
      CLUSTER_NAME=$(read_tfvar cluster_name)
      VM_NAMESPACE=$(read_tfvar vm_namespace)
    else
      die "Neither .env nor deploy-terraform/terraform.tfvars found — run full setup first (or pass --cluster <name> for per-cluster tfvars)"
    fi
  fi
...
KUBECONFIG
  ...
  # Update credential fields in tfvars (in-place). When --cluster <name> was
  # passed we write to deploy-terraform/<name>.tfvars; otherwise fall back to
  # the legacy deploy-terraform/terraform.tfvars.
  local cloud_cred_name="${CLUSTER_NAME}-harvester"
  local target_tfvars="${ACTIVE_TFVARS:-${SCRIPT_DIR}/deploy-terraform/terraform.tfvars}"
  if [[ -f "${target_tfvars}" ]]; then
    log_info "Updating credentials in ${target_tfvars#${SCRIPT_DIR}/}..."
    sed -i \
      -e "s|rancher_token.*=.*|rancher_token = \"${API_TOKEN}\"|" \
      -e "s|harvester_cloud_credential_name.*=.*|harvester_cloud_credential_name = \"${cloud_cred_name}\"|" \
      "${target_tfvars}"
    log_ok "Updated rancher_token and harvester_cloud_credential_name in ${target_tfvars#${SCRIPT_DIR}/}"
  fi
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --cluster)
        [[ $# -ge 2 ]] || die "--cluster requires a name argument"
        CLUSTER_FLAG="$2"; shift 2 ;;
      --cluster=*)
        CLUSTER_FLAG="${1#--cluster=}"; shift ;;
      -h|--help)
        usage ;;
      "")
        shift ;;
  ...
  [[ -n "${CLUSTER_FLAG}" && -n "${ENV_FILE}" ]] && \
    die "--cluster and --env are mutually exclusive"
  ...
  if [[ "${refresh_mode}" == true ]]; then
    if [[ -n "${CLUSTER_FLAG}" ]] || [[ -n "${ENV_FILE}" ]] || \
       [[ -f "${SCRIPT_DIR}/.env" ]] || \
       [[ -f "${SCRIPT_DIR}/deploy-terraform/terraform.tfvars" ]]; then
      refresh_credentials
    else
      log_warn "No --cluster/--env or .env or deploy-terraform/terraform.tfvars found — running full setup instead"
      full_setup
    fi
  else
    full_setup
